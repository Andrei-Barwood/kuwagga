...
setup_apple_silicon_env() {
    ...
    # Apple Silicon specific flags - simplified to avoid path issues
...
# Build GMP for Apple Silicon - simplified configuration to avoid libtool issues
build_gmp() {
    ...
    # Simplified configure to avoid absolute path issues
    local gmp_configure_args=(
        "--prefix=$ARCH_DIR"
        "--enable-static"
        "--disable-shared"
        "--disable-cxx"      # Disable C++ to avoid libgmpxx.la issues
    ...
    if ! ./configure "${gmp_configure_args[@]}" 2>&1 | tee configure.log; then
        error "GMP configure failed for Apple Silicon. Check configure.log"
    fi
...
build_flint() {
    ...
    # Set dependency paths explicitly
    export CPPFLAGS="$CPPFLAGS -I$ARCH_DIR/include"
    export LDFLAGS="$LDFLAGS -L$ARCH_DIR/lib"
