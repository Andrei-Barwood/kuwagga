...
setup_apple_silicon_env() {
    ...
    # Enhanced flags for Apple Silicon
    export CFLAGS="-arch arm64 -isysroot '$sdk_path' -mmacosx-version-min=11.0 -O3 -fno-stack-check"
    export CXXFLAGS="-arch arm64 -isysroot '$sdk_path' -mmacosx-version-min=11.0 -O3 -fno-stack-check -stdlib=libc++"
    export LDFLAGS="-arch arm64 -isysroot '$sdk_path' -mmacosx-version-min=11.0"
...
EOF
    ...
    # Test compilation
    log "Testing: $CC $CFLAGS $test_file -o $test_output"
    if ! $CC $CFLAGS "$test_file" -o "$test_output" 2>"$BUILD_DIR/compiler_test.log"; then
...
# Build GMP for Apple Silicon - ENHANCED
build_gmp() {
    ...
    # Configure for Apple Silicon - SIMPLIFIED
