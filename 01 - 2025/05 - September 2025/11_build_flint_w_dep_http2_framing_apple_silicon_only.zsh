...
# Use mktemp to create a safe temporary directory without spaces
SCRIPT_DIR="$(cd "$(dirname "${(%):-%N}")" && pwd)"
BUILD_BASE="$(mktemp -d -t flint_build)"
...
error() {
    ...
    [[ -n "$BUILD_BASE" && -d "$BUILD_BASE" ]] && rm -rf "$BUILD_BASE"
    exit 1
}
...
cleanup() {
    if [[ -n "$BUILD_BASE" && -d "$BUILD_BASE" ]]; then
        log "Cleaning up temporary build directory: $BUILD_BASE"
        rm -rf "$BUILD_BASE"
    fi
}
...
# Setup directories using system temp directory
setup_directories() {
    log "Setting up build directories..."
    log "Using system temporary directory: $BUILD_BASE"
    
    # Verify the temp directory was created successfully
    ...
    # Verify paths don't contain spaces (should never happen with mktemp)
    ...
    if [[ ! -w "$BUILD_DIR" ]]; then
        error "No write permission to build directory: $BUILD_DIR"
    fi
    
    cd "$BUILD_DIR"
    success "Build directories created in system temp: $BUILD_DIR"
}
...
setup_apple_silicon_env() {
    ...
    success "Temp build directory (no spaces): $BUILD_DIR"
}
...
# Build GMP with enhanced path handling and verification
build_gmp() {
    ...
    # Verify the library was built correctly with multiple search paths
    local gmp_lib_paths=(
...
# FIXED: Build FLINT for Apple Silicon with bootstrap support
build_flint() {
    ...
    # FLINT requires bootstrap to generate configure script
    log "Checking if FLINT configure script exists..."
    if [[ ! -f "./configure" ]]; then
        log "Configure script not found, running bootstrap..."
        
        # Check if bootstrap script exists
        if [[ -f "./bootstrap.sh" ]]; then
            log "Running FLINT bootstrap script..."
            if ! ./bootstrap.sh 2>&1 | tee bootstrap.log; then
                error "FLINT bootstrap failed. Check bootstrap.log"
            fi
        elif [[ -f "./autogen.sh" ]]; then
            log "Running FLINT autogen script..."
            if ! ./autogen.sh 2>&1 | tee autogen.log; then
                error "FLINT autogen failed. Check autogen.log"
            fi
        else
            # Try autoreconf as fallback
            log "No bootstrap script found, trying autoreconf..."
            if command -v autoreconf >/dev/null 2>&1; then
                if ! autoreconf --install --force --verbose 2>&1 | tee autoreconf.log; then
                    error "FLINT autoreconf failed. Check autoreconf.log"
                fi
            else
                error "No bootstrap script found and autoreconf not available. Please install autotools (autoconf, automake, libtool)"
            fi
        fi
        
        # Verify configure script was created
        if [[ ! -f "./configure" ]]; then
            log "=== Contents of FLINT source directory ==="
            ls -la .
            error "Configure script was not generated after bootstrap"
        fi
        
        success "FLINT configure script generated successfully"
    else
        log "Configure script already exists"
    fi
    ...
    if ! ./configure "${flint_configure_args[@]}" 2>&1 | tee configure.log; then
        log "=== Last 50 lines of configure.log ==="
...
EOF
    ...
    # Handle spaces in script directory path properly
...
# Check dependencies - ENHANCED for autotools
check_dependencies() {
    ...
    # Check for autotools (needed for FLINT bootstrap)
    if ! command -v autoreconf >/dev/null 2>&1; then
        warning "autoreconf not found. Installing autotools recommended: brew install autoconf automake libtool"
        warning "Will try to use FLINT's bootstrap script instead"
    else
        log "Found autoreconf: $(which autoreconf)"
    fi
