...
# Set Apple Silicon environment variables - CORRECTED
setup_apple_silicon_env() {
    ...
    # Set up compiler toolchain - FIXED
    ...
    # CORRECTED: Single arch flag and proper SDK setup
    export CFLAGS="-arch arm64 -isysroot $sdk_path -mmacosx-version-min=11.0 -O3 -fno-stack-check"
    export CXXFLAGS="-arch arm64 -isysroot $sdk_path -mmacosx-version-min=11.0 -O3 -fno-stack-check"
    export LDFLAGS="-arch arm64 -isysroot $sdk_path -mmacosx-version-min=11.0 -Wl,-ld_classic"
    export CPPFLAGS="-arch arm64 -isysroot $sdk_path"
    ...
    success "CC: $CC"
    success "CFLAGS: $CFLAGS"
}

# Build GMP for Apple Silicon - CORRECTED
build_gmp() {
    ...
    if ! $CC $CFLAGS -c -o test.o /dev/null 2>/dev/null; then
        error "Compiler test failed. Check CFLAGS and compiler setup."
    fi
    rm -f test.o
    
    # Configure for Apple Silicon - CORRECTED
    ...
    if [[ ! -f "$ARCH_DIR/lib/libgmp.a" ]]; then
        error "GMP static library not found at $ARCH_DIR/lib/libgmp.a"
    fi
    
    # Check architecture of built library
    if command -v file >/dev/null 2>&1; then
        log "GMP library architecture:"
        file "$ARCH_DIR/lib/libgmp.a"
    fi
